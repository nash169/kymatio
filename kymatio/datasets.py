...
atom_charges = dict(H=1, C=6, O=8, N=7, S=16, F=9)
...
qm9_url = "https://qmml.org/Datasets/gdb9-14b.zip"


def read_xyz_qm9(filename):
    """Reads xyz files that are used for storing molecule configurations.
    ...
    for raw_molecule_txt in raw_molecule_txts:
        s = raw_molecule_txt.split("\n")
        n_atoms.append(int(s[0]))
        energies.append(s[1].split()[2:])
        atom_positions = []
        molecule_charges = []
        charges.append(molecule_charges)
        positions.append(atom_positions)
        for _, row in zip(range(n_atoms[-1]), s[2:]):
            atom_type, *str_position = [x for x in row.split() if x]
            molecule_charges.append(atom_charges[atom_type])
            pos = np.array(list(map(float, str_position[:-1])))
            atom_positions.append(pos)
...
def fetch_qm9(align=True, cache=True):
    """Fetches the GDB9-14 dataset"""

    if cache:
        cache_path = get_cache_dir("qm9")
        if align:
            aligned_filename = os.path.join(cache_path, "qm9_aligned.npz")
            if os.path.exists(aligned_filename):
                f = np.load(aligned_filename)
                return dict(**f)

        # load unaligned if existent, align if required
        unaligned_filename = os.path.join(cache_path, "qm9.npz")
        if os.path.exists(unaligned_filename):
            f = np.load(unaligned_filename)
            if align:
                _pca_align_positions(f['positions'], f['charges'], inplace=True)
    ...
    path = get_dataset_dir("qm9")
    qm9_file_bz2 = os.path.join(path, "dsgdb9nsd.xyz.bz2")
    qm9_file = os.path.join(path, "dsgdb9nsd.xyz")
    if not os.path.exists(qm9_file):
        qm9_zipfile = os.path.join(path, "gdb9-14b.zip")
        if not os.path.exists(qm9_zipfile):
            _download(qm9_url, qm9_zipfile)
            import zipfile
            with zipfile.ZipFile(qm9_zipfile, "r") as zipref:
                zipref.extractall(path)
            import bz2
            with bz2.BZ2File(qm9_file_bz2, "rb") as f_in, open(qm9_file, "wb") as f_out:
                f_out.write(f_in.read())

    qm9 = read_xyz_qm9(qm9_file)
    if cache:
        np.savez(unaligned_filename, **qm9)

    if align:
        _pca_align_positions(qm9['positions'], qm9['charges'], inplace=True)
        if cache:
            np.savez(aligned_filename, **qm9)
